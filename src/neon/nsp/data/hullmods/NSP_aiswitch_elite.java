...
import static neon.nsp.data.NSP_reference_sheet.*;

public class NSP_aiswitch_elite extends BaseHullMod {
    ...
    private void applyDecorativeWeaponBasedOnMode(ShipAPI ship, MutableShipStatsAPI stats, ShipVariantAPI variant) {
        ...
        boolean isAutomatedMode = variant.hasHullMod(NSP_AISWITCHAUTOMATED_ELITE);
    ...
    private void handleAutomationSwitching(ShipVariantAPI variantStats) {
        if (variantStats.hasTag(Tags.AUTOMATED)) {
            if (!variantStats.hasHullMod(NSP_AISWITCHAUTOMATED_ELITE)) {
            ...
            if (!variantStats.hasHullMod(NSP_AISWITCHMANUAL)) {
                ...
                variantStats.addMod(NSP_AISWITCHAUTOMATED_ELITE);
