...
public class NSP_TrebuchetOnHitEffect implements OnHitEffectPlugin {
    ...
    public void onHit(DamagingProjectileAPI projectile, CombatEntityAPI target, Vector2f point, boolean shieldHit, ApplyDamageResultAPI damageResult,
        ...
        List<ShipAPI> targets = NSP_Util.getShipsWithinRange(point, AREA_EFFECT);

        NSP_Util.filterObscuredTargets(target, point, targets, true, true, false);
    ...
    public static final class EMPBlast extends BaseCombatLayeredRenderingPlugin {
        ...
        public void advance(float amount) {
            ...
            float coreSize = NSP_Util.lerp(radius * coreSizeFractionStart, radius * coreSizeFractionEnd, level) * 2f;
            float fringeSize = NSP_Util.lerp(0f, radius, level) * 2f;
            float alpha = Math.min(1f, Math.max(0f, NSP_Util.lerp(brightness, 0f, (float) Math.pow(level, 1.5))));
