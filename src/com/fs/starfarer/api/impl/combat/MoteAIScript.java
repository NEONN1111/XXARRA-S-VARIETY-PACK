...
//			}
	...
	public void advance(float amount) {
		...
		// 3. Closest valid enemy ship/missile/fighter
		ShipAPI source = missile.getSource();
		...
		// Otherwise use shared attractor target
		else if (data.attractorLock != null && data.attractorLock.isAlive() && !data.attractorLock.isPhased()) {
...
//		}
	...
	// NEW: Modified to prioritize ship's current target, then shared attractor, then closest enemy
	protected void acquireNewTargetIfNeeded() {
		...
		// Priority 3: Find closest valid enemy (original behavior)
		CombatEngineAPI engine = Global.getCombatEngine();
